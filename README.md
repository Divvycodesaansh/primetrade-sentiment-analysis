...
Primetrade.ai · Data Science Intern Assignment · Divyaansh Monga
...
Two datasets were used: 211,224 Hyperliquid trade records from 31 accounts across Jan to Dec 2024, and a daily Bitcoin Fear/Greed index with a 0 to 100 score and classification label. The datasets were merged on date after parsing the IST timestamp using dayfirst=True.
Critical cleaning decision: Only 16,754 of the 211,224 rows are closing trades where Direction contains the word Close. These are the only rows with realized PnL. All performance metrics were computed exclusively on closing trades to avoid diluting averages and misrepresenting win rates.
Sentiment was collapsed from 5 classes to 3 — Fear, Neutral, and Greed — for statistical power. The merge yielded 52,491 matched rows across 340 dates.

Metrics built:

Daily PnL per trader (sum of closing trade PnL)

Win rate per trader per day (percentage of closing trades with PnL above 0)

Long/Short ratio (BUY count divided by SELL count per day)
...
Important note on bias: 2024 was a Bitcoin bull year. 263 of 365 days were classified as Greed or Extreme Greed. Fear-day findings are based on only 59 days and carry lower statistical power.
...
When Fear & Greed score < 35 → cap new position size at 40% of 7-day average
→ Reduces the 5 worst single-day losses by ~60% (total saving ~$284,000)

Rule 2 – Greed Profit-Lock Rule
When Fear & Greed score > 80 → stop opening new positions after +2% daily PnL is reached
